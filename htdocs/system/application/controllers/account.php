...
*/
	...
	class Account extends Controller {
		...
		function index($action = 0) {
			...
			if($action == 2) success("Seu apelido foi criado! obrigado!");
			if($action == 3) success("Você Exedel a quantidade máxima de caracteres por conta.");
			...
			try { $account->find($_SESSION['name']); } catch(Exception $e) {show_error('Houve um problema durante o carregamento da conta. Err code: 220212072010 mais detalhes: '.$e->getMessage());}
		...
		*/
		...
		function _emailExists($email) {
			...
			if($this->account_model->emailExists($email)) {
				$this->form_validation->set_message('_emailExists', 'Este e-mail já está sendo usado por outra conta.');
				return false;
			}
			else
				return true;
		...
		function _characterExists($name) {
			...
			if($this->character_model->characterExists($name)) {
				$this->form_validation->set_message('_characterExists', 'Já existe um personagem com esse nome, por favor escolha outro.');
				return false;
			}
			else
				return true;
		...
		function _checkDelay() {
			...
			if($config['accountDelay']) {
				if(@(time()-$_SESSION['accountDelay']) > 240) {
					...
					$this->form_validation->set_message('_checkDelay', 'Você não pode criar outra conta agora, por favor espere.');
					return false;
				}
			}
			else
	...
	function strFirst($name) {
		...
		function generate_recovery_key() {
           ...
           alert("<b>You can't generate a new KEY</b>");
        return false;
}  
			if($_POST) {
				$data['info'] = '';
