	...
	if($deaths->rowCount())
	{
		...
		foreach($deaths as $death)
		{
			...
			foreach($killers as $killer)
			{
				...
				echo (!empty($killer['player_name']) ? ($i == 1 ? "{$str} No Level {$death['level']} Por " : ($i == count($killers) ? " and " : ", ")).(!empty($killer['monster_name']) ? "{$killer['monster_name']} of " : "").($killer['player_exists'] == 0 ? "<a href=\"".WEBSITE."/index.php/character/view/{$killer['player_name']}\">{$killer['player_name']}</a>" : $killer['player_name']) : ($i == 1 ? "Morto no Level {$death['level']} por {$killer['monster_name']}" : ($i == count($killers) ? " and {$killer['monster_name']}" : ", {$killer['monster_name']}")));
			}
