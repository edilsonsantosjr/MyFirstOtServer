...
void MonsterType::reset()
{
	...
	experience = defense = armor = lookCorpse = corpseUnique = corpseAction = conditionImmunities = damageImmunities = sagaValue = 0;
		...
		else if(!xmlStrcmp(p->name, (const xmlChar*)"flags"))
		{
			for(xmlNodePtr tmpNode = p->children; tmpNode; tmpNode = tmpNode->next)
			{
				if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"flag"))
				{
					...
					if (readXMLInteger(tmpNode, "sagaValue", intValue))
						mType->sagaValue = intValue;

					if (readXMLString(tmpNode, "sagaStorage", strValue))
						mType->sagaStorage = strValue;
