...
ReturnValue Combat::canDoCombat(const Creature* attacker, const Creature* target, bool isAggressive)
{
	...
	if(const Player* targetPlayer = target->getPlayer())
	{
		...
		if(attacker->getMonster() && attacker->getMonster()->getSagaStorage() != "0")
		{
			std::string saga;		
			targetPlayer->getStorage(attacker->getMonster()->getSagaStorage(), saga);
			if(attacker->getMonster()->getSagaValue() != atoi(saga.c_str()))	
				return RET_YOUMAYNOTATTACKTHISPLAYER;
		}
	...
	else if(target->getMonster())
	{
		...
		if((attackerPlayer = attacker->getPlayer()) || (attackerPlayer = attacker->getPlayerMaster()))
		{
			...
			if(target->getMonster()->getSagaStorage() != "0")
			{
				std::string saga;		
				attackerPlayer->getStorage(target->getMonster()->getSagaStorage(), saga);
				if(target->getMonster()->getSagaValue() != atoi(saga.c_str()))	
					return RET_YOUMAYNOTATTACKTHISCREATURE;
			}
		}
	}
