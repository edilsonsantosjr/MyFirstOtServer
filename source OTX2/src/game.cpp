...
std::string _text = asLowerCaseString(text);
   ...
   std::string words []= {"resetar", "resetei", "resetado", "resetou", "resetando", "reseta", "reset", "reseted", "resete", "rezetar", "rezetado", "rezetou", "rezetando", "rezeta", "rezet", "rezeted", "rezete", "rezetei", ".net", ".com", ".ddns", ".servegame", ".org", "baiak", ".no-ip", ".pl", ".biz"};
   int ii, length;
   ...
   for(ii=0; ii < int(length); ii++)
   {
      if (int(_text.find(words[ii])) > 0 || _text == words[ii])
		{
			internalCreatureSay(player, MSG_SPEAK_SAY, "O melhor OTServer da atualidade.", false, NULL); 
             return false;
          break;
		}
	}
